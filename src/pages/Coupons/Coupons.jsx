...
const Coupons = () => {
  ...
  const formatDateTime = (dateTime) => {
    ...
    const formattedTime = moment(dateTime).format('h:mm:ss a');
    return {
      date: formattedDate,
      time: formattedTime,
    };
  ...
  const modifyCouponList = (originalList) => {
    const formattedCouponList = originalList.map((coupon) => {
      const formattedStart = formatDateTime(coupon.startDate);
      const formattedExpiry = formatDateTime(coupon.expiryDate);
      const obj = {
        ID: coupon.id,
        ...
        'Start Date': formattedStart.date,
        'Start Time': formattedStart.time,
        'Expiry Date': formattedExpiry.date,
        'Expiry Time': formattedExpiry.time,
      };
      return obj;
    });
  ...
  console.log(modifiedCouponList);
  ...
  const hasCouponAccess =
    userData && userData.user && userData.user.hasCouponAccess
      ? userData.user.hasCouponAccess
      : false;
