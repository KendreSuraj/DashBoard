 ...
 const role = JSON.parse(localStorage.getItem('userData'))?.user?.role;
    ...
    useEffect(() => {
        setBooking(prevBooking => ({
            ...prevBooking, previousSlotDate: clientInfo?.slotDate,
            previousSlotTime: clientInfo?.slotTime, city: clientInfo.city,
    ...
    const getTimeSlots = async (date, index) => {
        try {
            ...
            if (response?.status === 200) {
                setIsLoading(false);
                setTimeSlot(response?.data?.slots.sort((a, b) => {
            ...
            <div>
                ...
                <div style={{ textAlign: 'center', marginTop: '20px' }}>
                    {!isLoading && timeSlot?.length > 0 ? (
                                ...
                                {timeSlot.map((item, index) => (
                                    <li
                                        ...
                                        style={{
                                            ...
                                            margin: '5px',
                                            cursor: 'pointer',
                                            background: activeOption1 === index ? '#ccc' : 'transparent',
                                            padding: '5px 10px',
                                            borderRadius: '5px',
                                            fontWeight: 'bold',
                                            border: '1px solid #ccc'
                        ...
                        <h3>
                            {!isLoading && timeSlot?.length === 0 && "Please Select Booking date"}
                        </h3>
                    )}
                </div>
            </div>
